...
#define REGISTER_ARC(x) struct REGISTER_ARC_NAME(x) \
    { \
        ...
        { \
            if(g_ArcInfo.once==0)\
                g_ArcInfo.once = new std::once_flag();\
            std::call_once(*g_ArcInfo.once, RegisterArc,(const CArcInfo *)&g_ArcInfo); \
        } \
    }; \
    static REGISTER_ARC_NAME(x) g_RegisterArc; \
    void registerArc##x() { static REGISTER_ARC_NAME(x) g_RegisterArc; }

#define REGISTER_ARC_DEC_SIG(x)\
    void callOnce##x() \
    {\
        g_ArcInfo.Signature[0]--; \
        RegisterArc(&g_ArcInfo); \
    }\
    struct REGISTER_ARC_NAME(x) \
    { \
        REGISTER_ARC_NAME(x)() { \
            if(g_ArcInfo.once==0)\
                g_ArcInfo.once= new std::once_flag();\
            std::call_once(*g_ArcInfo.once, callOnce##x); \
        } \
    }; \
    static REGISTER_ARC_NAME(x) g_RegisterArc; \
