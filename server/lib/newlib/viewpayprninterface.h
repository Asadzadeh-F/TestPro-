#ifndef VIEWPAYPRNINTERFACE_H
#define VIEWPAYPRNINTERFACE_H

#include <QObject>
#include "viewpayplib.h"
#ifdef VIEWPAY_DEF
...
#define VP_BARCODE_WIDTH_ERR			10//条码宽度超限
#define VP_BARCODE_HEIGHT_ERR			11//条码高度超限
#define VP_BARCODE_FONT_POS_ERR			12//文字位置错误
#define VP_BARCODE_TEXT_FONT_TYPE_ERR	13//字体错误
#define VP_BARCODE_DATA_TYPE_ERR		14//数据类型错误，输入了非数字数据
#define VP_BARCODE_DATA_FORMAT_ERR		15//数据格式错误，如：数据位数错误
#define VP_BARCODE_CHKCODE_ERR			16//校验码错误
#define VP_BARCODE_NO_SPACE_ERR			17//条码文字与附加码之间没有空格
#define VP_BARCODE_NO_BARCODE_ERR		18//没有输入条码
...
class ViewpayPrnInterface : public QObject
{
    Q_OBJECT
    
public:
    explicit ViewpayPrnInterface(QObject *parent = 0);
    ~ViewpayPrnInterface();
public:
    void PrnSetAlign(int i=0);
